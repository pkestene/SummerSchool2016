...
void diffusion(Field2d U, Field2d S, const DataWarehouse &dw)
{
  ...
  // the interior grid points
  {
    ...
    using range_t = Kokkos::Experimental::MDRangePolicy< Kokkos::Experimental::Rank<2>,
							 Kokkos::IndexType<int> >;
    range_t range( {0,0}, {nx,ny} );
  ...
  // the north boundary (plus NE and NW corners)
  {
    ...
    Kokkos::parallel_for( nx, KOKKOS_LAMBDA( int index ) {
	if (index == 0) {
	  int i = 0; // NW corner
	  S(i,j) = -(4. + alpha) * U(i,j)
	    + U(i+1,j) + U(i,j-1)
