...
public:
  ...
  void ss_cg(Field1d x, Field1d const b, const int maxiters, const double tol, bool& success);

private:
  bool cg_initialized;
  DataWarehouse &dw;
  Field1d r, Ap, p, Fx, Fxold, v, xold; // 2d
...
// x and y are vectors on length N
// alpha is a scalar
void ss_axpy(Field1d y, double alpha, Field1d x);

// computes y = x + alpha*(l-r)
// y, x, l and r are vectors of length N
// alpha is a scalar
void ss_add_scaled_diff(Field1d y, Field1d x, double alpha,
			Field1d l, Field1d r);

// computes y = alpha*(l-r)
// y, l and r are vectors of length N
// alpha is a scalar
void ss_scaled_diff(Field1d y, double alpha,
		    Field1d l, Field1d r);

// computes y := alpha*x
// alpha is scalar
// y and x are vectors on length n
void ss_scale(Field1d y, double alpha, Field1d x);

// computes linear combination of two vectors y := alpha*x + beta*z
// alpha and beta are scalar
// y, x and z are vectors on length n
void ss_lcomb(Field1d y, double alpha, Field1d x, double beta,
	      Field1d z);

// copy one vector into another y := x
// x and y are vectors of length N
void ss_copy(Field1d y, Field1d x);
